...
Vagrant.configure(2) do |config|
  ...
  config.vm.synced_folder "/Users/yjiang/data/vagrant", "/vagrant_data"
  ...
  # Dev1 VM
  config.vm.define :dev1 do |d1|
    d1.vm.hostname = "dev1.kintoki.mt"
    d1.vm.network :private_network, ip: "192.168.0.1"
    d1.vm.provider :virtualbox do |vb|
      vb.memory = "8192"
    end

    d1.vm.network "forwarded_port", guest: 8080, host: 8081
  end


  # Ambari1
  config.vm.define :ambari1 do |a1|
    a1.vm.hostname = "ambari1.kintoki.mt"
  ...
  config.vm.define :master1 do |m1|
    m1.vm.hostname = "master1.kintoki.mt"
  ...
  config.vm.define :slave1 do |s1|
    s1.vm.hostname = "slave1.kintoki.mt"
  ...
  config.vm.define :slave2 do |s2|
    s2.vm.hostname = "slave2.kintoki.mt"
