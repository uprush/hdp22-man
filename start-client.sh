...
CLIENT_FQDN='ambari1.kintoki.mt'
...
if [[ $? -ne 0 ]]; then
  echo "Starting client, FQDN: ${CLIENT_FQDN}"
  vagrant up ambari1
fi
...
rm -f $DEMO_HOME/conf/cluster/client
mkdir -p $DEMO_HOME/conf/cluster


echo $CLIENT_FQDN > $DEMO_HOME/conf/cluster/client
echo "Client private FQDN: `cat $DEMO_HOME/conf/cluster/client`"
